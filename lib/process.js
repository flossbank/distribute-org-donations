...
const adjustAmount = (amount) => amount <= 0 ? 0 : ((amount * 0.96) - 30) * 1000

exports.process = async ({ log, record, db, retriever, resolver, dynamo }) => {
  const {
    amount, // amount of donation in cents
  ...
  const donationAmount = redistributedDonation ? amount * 1000 : adjustAmount(amount)
  ...
  if (targetPackageId) {
    ...
    // if we don't have a target package ID, we will connect to the orgranization's version control
  ...
  if (!redistributedDonation) {
    ...
    await db.updateDonatedAmount({ organizationId, amount: amount * 1000 })
  }
