exports.process = async ({ log, record, db, retriever, resolver, dynamo }) => {
  ...
  const { name, accessToken } = await db.getOrgAccessToken({ organizationId })
  ...
  const packageManifests = await retriever.getAllManifestsForOrg(name, searchPatterns, accessToken)
