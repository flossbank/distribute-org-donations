...
class GithubRetriever {
        ...
        afterResponse: [
          async (response) => {
            ...
            if (rateLimits && rateLimits.remaining <= 5) {
              this.log.warn('Rate limited while requesting (%s); continuing at %d (%s)', response.requestUrl, rateLimits.reset.getTime(), rateLimits.reset.toString())
  ...
  async searchForManifests (repo, searchPatterns, token) {
    ...
    const options = {
      searchParams: { q: `${filenames} repo:${repo.full_name}` },
      pagination: {
        ...
        paginate: (response) => { // copied from https://github.com/sindresorhus/got/blob/v11.8.2/source/index.ts#L80
          if (!response.headers.link) {
            this.log.warn(`Got no next page for ${response.requestUrl}`)
            return false
          }
          ...
          if (next) {
            ...
            this.log.warn(`Fetching next page for ${response.requestUrl} => ${options.url}`)
            return options
          }

          this.log.warn(`Link header present, but no next page found: ${response.headers.link}`)
          return false
        }
      },
      token
